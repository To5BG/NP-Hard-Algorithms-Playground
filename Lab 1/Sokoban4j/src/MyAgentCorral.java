...
// */
//public class MyAgentCorral extends ArtificialAgent {
...
//        MyAgentCorral.board = board.makeBoardSlim();
//        searchedNodes = 0;
//        // Find longer side for avoiding collisions on box bitmask
//        dim = Math.max(board.width(), board.height());
//        goals = findEntities(MyAgentCorral.board, STile.PLACE_FLAG);
//        dsd = new DeadSquareDetector(MyAgentCorral.board);
...
//                if (STile.isWall(MyAgentCorral.board.tiles[i][j])) walls[i * dim + j] = true;
...
//        while (corralRisk <= MyAgentCorral.board.boxCount) {
